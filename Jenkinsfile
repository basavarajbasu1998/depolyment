...
pipeline {
    ...
    stages {
        ...
        stage('Build Backend Docker Image') {
            steps {
                dir(params.BACKEND_DIR) {
                    bat "docker build -t %DOCKER_HUB_USERNAME%/%DOCKER_HUB_BACKEND_REPO%:%DOCKER_IMAGE_TAG% ."
                }
            }
        }
        ...
        stage('Build Frontend Docker Image') {
            steps {
                dir(params.FRONTEND_DIR) {
                    bat "docker build -t %DOCKER_HUB_USERNAME%/%DOCKER_HUB_FRONTEND_REPO%:%DOCKER_IMAGE_TAG% ."
                }
            }
        }
        ...
        stage('Push Images to Docker Hub') {
            steps {
                script {
                    ...
                    docker push %DOCKER_HUB_USERNAME%/%DOCKER_HUB_BACKEND_REPO%:%DOCKER_IMAGE_TAG%
                    docker push %DOCKER_HUB_USERNAME%/%DOCKER_HUB_FRONTEND_REPO%:%DOCKER_IMAGE_TAG%
                    """
                }
            }
        }
