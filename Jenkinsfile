...
pipeline {
    ...
    stages {
        ...
        stage('Build Backend Docker Image') {
            steps {
                dir(params.BACKEND_DIR) {
                    powershell "docker build -t ${params.DOCKER_HUB_USERNAME}/${params.DOCKER_HUB_BACKEND_REPO}:${params.DOCKER_IMAGE_TAG} ."
                }
            }
        }
        ...
        stage('Build Frontend Docker Image') {
            steps {
                dir(params.FRONTEND_DIR) {
                    powershell "docker build -t ${params.DOCKER_HUB_USERNAME}/${params.DOCKER_HUB_FRONTEND_REPO}:${params.DOCKER_IMAGE_TAG} ."
                }
            }
        }
        ...
        stage('Push Images to Docker Hub') {
            steps {
                powershell """
                    echo '${params.DOCKER_HUB_PASSWORD}' | docker login -u ${params.DOCKER_HUB_USERNAME} --password-stdin
                    docker push ${params.DOCKER_HUB_USERNAME}/${params.DOCKER_HUB_BACKEND_REPO}:${params.DOCKER_IMAGE_TAG}
                    docker push ${params.DOCKER_HUB_USERNAME}/${params.DOCKER_HUB_FRONTEND_REPO}:${params.DOCKER_IMAGE_TAG}
                """
            }
        }
    }
